...
int get_command(char *inputBuffer, int size, char *args[], int *background, int vecesMatados, int matados)
{
    ...
    *background=0;
    ...
    start = -1;
    if (length == 0) {
        printf("\nVecesQueLimpia: %d, ProcesosMatados: %d\nBye\n", vecesMatados, matados);
...
job * new_job(pid_t pid, const char * command, enum job_state state)
{
    ...
    aux->next = NULL;
    aux->args = NULL;
    aux->respawnable = NULL;
    aux->segundosDeVida = 0;
    aux->segundosDelay = 0;
    return aux;
}
...
enum status analyze_status(int status, int *info)
{
    ...
    if (WIFSIGNALED(status)) { // el proceso ha terminado por una seÃ±al
...
// -----------------------------------------------------------------------
// cambia la accion de las seÃƒÂ±ales relacionadas con el terminal
